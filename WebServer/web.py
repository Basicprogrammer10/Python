...
def collect():
    ...
    while True:
        ...
        open(outputFile, mode='a').write(
            str(datetime.datetime.now())+","+str(data)+"\n")
        time.sleep(waitTimeMin*60)
...
<script>
    ...
    <canvas data-type="radial-gauge"
            ...
            data-highlights='[
                { "from": 0, "to": 65, "color": "rgba(0,0,255)" },
                { "from": 65, "to": 75, "color": "rgba(0,255,0)" },
                { "from": 75, "to": 100, "color": "rgba(255,30,0)" }
            ]'
...
if __name__ == "__main__":
    ...
    if is_admin:
        ...
        print(colored("You need to run this script with SUDO", 'red'))
        print(colored("Try Running 'sudo python3 Temp.py'\n", 'red'))
    try:
        config = configparser.ConfigParser()
        config.read("Config.ini")
        hostName = config.get('Web Server', 'hostName')
        serverPort = int(config.get('Web Server', 'serverPort'))
        waitTimeMin = int(config.get('Data', 'waitTimeMin'))
        outputFile = config.get('Data', 'outputFile')
    except:
        print(colored("Error Reading Config File", 'red'))
        print(colored("Should I create a Config File?", 'yellow'))
        if input(colored('(Y/N)', 'yellow')) == 'Y':
            try:
                config = open('Config.ini', 'w')
                config.write(defultconfig)
                print(colored("Config File Was Made!", 'green'))
                try:
                    config = configparser.ConfigParser()
                    config.read("Config.ini")
                    hostName = config.get('Web Server', 'hostName')
    ...
    try:
        ...
        print(colored("Web Server couldn't Start", 'red'))
    ...
    try:
        ...
        pass
    print(colored("Server Is Stopping", 'blue'))
    quit()