...
ap.add_argument("-t", "--text", required=False, help="The Text (EX: -t 'KHOOR ZRUOG')")
ap.add_argument("-f", "--file", required=False, help="File name to output to. (EX: -t 'KHOOR ZRUOG' -f 'text')")
ap.add_argument("-d", "--duck", required=False, help="Searches Duckduckgo! (EX: -d 'Nose')")
ap.add_argument("-nose", "--nose", required=False, help="NOSE! (EX: -n')", action='store_true')
ap.add_argument("-v", "--version", required=False, help="Displays Version INFO (EX: -v')", action='store_true')
ap.add_argument("-u", "--unmix", required=False, help="Un Mix a Word (EX: -u estt')")
...
if args["version"] != False:
    print(colored("BrainChaseSolver™", 'blue') + " | " + colored("V" + str(ver), 'yellow') + " | " + colored("By: Connor Slade", 'green'))
    ...
    try:
        try:
            ...
            formatwords = []
            print(colored("Reformatting Word Database (This may take some time...)", 'yellow'))
            ...
            for i in range(len(shuffleld)):
                print(colored("Checking ", 'cyan') + colored("#" + str(i), 'magenta') + " | " + colored(shuffleld[i], 'blue'))
                    ...
                    if str(shuffleld[i]) in str(formatwords[b]):
                        if len(str(formatwords[b])) <= len(str(shuffleld[i])):
                            if str(formatwords[b]) not in workingarray:
                                ...
                                print(colored("    >", 'green') + colored(str(formatwords[b]),'blue'))
    ...
    for i in range(26):
        ...
        if shift < 10:
            print(colored("Shift: 0" + str(shift), 'magenta') + colored(" | ", 'white') + colored(plain_text, 'blue'))
        else:
            print(colored("Shift: " + str(shift), 'magenta') + colored(" | ", 'white') + colored(plain_text, 'blue'))
        if DoFile == True:
            if shift < 10:
    ...
    print(colored("You have entered the BrainChaseSolver™ Shell", "blue") + colored("\nType `exit` to exit\nType `help` for help\n", 'green'))
    while True:
        ...
        if command == 'version':
            ...
            try:
                ...
                for i in range(26):
                    ...
                    if shift < 10:
                        print(colored("Shift: 0" + str(shift), 'magenta') + " | " + colored(plain_text, 'blue'))
                    else:
                        print(colored("Shift: " + str(shift), 'magenta') + " | " + colored(plain_text, 'blue'))
                    if DoFile == True:
                        if shift < 10:
                            file.write("Shift: 0" + str(shift) + " " + plain_text + "\n")
                        else:
